...
function App() {
  ...
  useEffect(() => {
    const fetchNASAData = async () => {
      try {
        const [envData, wthData, status] = await Promise.all([
          NASAAPI.getEnvironmentalData(),
          NASAAPI.getWeatherData(),
          NASAAPI.getSystemStatus()
        ]);
        
        setEnvironmentData(envData);
        setWeatherData(wthData);
        setSystemStatus(status);
        
        // Update layer opacities based on real data
        if (envData) {
          // Adjust temperature layer opacity based on anomaly
          const tempAnomaly = Math.abs(envData.temperature?.anomaly || 0);
          const tempOpacity = Math.min(0.9, 0.5 + tempAnomaly * 0.2);
          setLayerOpacities(prev => ({
            ...prev,
            temperature: tempOpacity
          }));
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    activeLayers.forEach(layerId => {
      ...
      if (layerMesh) {
        earthRef.current.add(layerMesh);  // Parented to Earth
    ...
    function createTemperatureLayer(layerId) {
      ...
      // Use real temperature data if available
      const currentTemp = environmentData?.temperature?.global_average || 15.0;
      const tempAnomaly = environmentData?.temperature?.anomaly || 1.0;
      ...
      const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            // Temperature gradient based on real data
            float lat = abs(vUv.y - 0.5) * 2.0;
            float baseTemp = (currentTemp - 10.0) / 30.0; // Normalize to 0-1 range
            float temp = baseTemp + (1.0 - lat * 0.8);
            
            // Enhance with anomaly data
            float anomalyEffect = tempAnomaly * 0.1;
            temp += anomalyEffect;
            ...
            // Highlight areas with high anomaly
            if (anomalyEffect > 0.05) {
              color = mix(color, vec3(1.0, 0.5, 0.0), 0.3);
            }
      ...
      const animate = () => {
        if (material.uniforms.time && dataLayersRef.current.has(layerId)) {
          ...
          // Update temperature data in real-time
          if (environmentData?.temperature) {
            material.uniforms.currentTemp.value = environmentData.temperature.global_average;
            material.uniforms.tempAnomaly.value = environmentData.temperature.anomaly;
          }
          requestAnimationFrame(animate);
        }
      };
    ...
    function createVegetationLayer(layerId) {
      ...
      // Use real vegetation data if available
      const ndvi = environmentData?.vegetation?.ndvi_global || 0.4;
      const healthIndex = environmentData?.vegetation?.health_index || 75.0;
      ...
      const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            float baseVegetation = max(tropics, temperate * 0.7);
            
            // Use real NDVI data to influence vegetation
            float vegetation = baseVegetation * ndvi * 2.0;
            ...
            vec3 healthyColor = vec3(0.2, 0.8, 0.3);
            vec3 stressedColor = vec3(0.8, 0.8, 0.2);
            vec3 unhealthyColor = vec3(0.8, 0.4, 0.1);
            
            vec3 color = mix(unhealthyColor, healthyColor, healthIndex / 100.0);
            color = mix(color, stressedColor, 0.3);
      ...
      const animate = () => {
        if (material.uniforms.time && dataLayersRef.current.has(layerId)) {
          ...
          // Update vegetation data in real-time
          if (environmentData?.vegetation) {
            material.uniforms.ndvi.value = environmentData.vegetation.ndvi_global;
            material.uniforms.healthIndex.value = environmentData.vegetation.health_index;
          }
          requestAnimationFrame(animate);
        }
      };
    ...
    function createCO2Layer(layerId) {
      ...
      // Use real CO2 data if available
      const co2Value = environmentData?.co2?.value || 417.0;
      const co2Intensity = Math.min(1.0, (co2Value - 300) / 200); // Normalize CO2 level
      ...
      const material = new THREE.ShaderMaterial({
        ...
        side: THREE.DoubleSide,
        uniforms: {
          opacity: { value: layerOpacities[layerId] || 0.5 },
          co2Intensity: { value: co2Intensity }
        },
        vertexShader: `
          varying vec2 vUv;
          void main() {
        ...
        fragmentShader: `
          uniform float opacity;
          uniform float co2Intensity;
          varying vec2 vUv;
          
          void main() {
            // CO2 concentration visualization
            float industrialZones = sin(vUv.x * 20.0) * sin(vUv.y * 15.0) * 0.5 + 0.5;
            float co2Density = industrialZones * co2Intensity;
            
            // Orange to red gradient based on CO2 levels
            vec3 lowCO2 = vec3(1.0, 0.8, 0.0);
            vec3 highCO2 = vec3(1.0, 0.0, 0.0);
            vec3 color = mix(lowCO2, highCO2, co2Density);
            
            gl_FragColor = vec4(color, opacity * co2Density);
          }
        `
      });
    ...
    function createAirQualityLayer(layerId) {
      ...
      // Use real air quality data if available
      const airQuality = weatherData?.alerts?.air_quality || 'moderate';
      let airQualityColor = 0x666666;
      
      switch (airQuality) {
        case 'good':
          airQualityColor = 0x00ff00;
          break;
        case 'moderate':
          airQualityColor = 0xffff00;
          break;
        case 'poor':
          airQualityColor = 0xff6600;
          break;
        case 'hazardous':
          airQualityColor = 0xff0000;
          break;
      }
  ...
  }, [activeLayers, environmentData, weatherData, layerOpacities]);
    ...
    <div className="app-container">
      ...
      <div className="ui-overlay">
        ...
        <div className="bottom-left-text">
          GaiaNet Digital Earth Twin
          {environmentData && (
            <div className="data-source">
              Live Data: {environmentData.data_sources?.join(', ') || 'NASA APIs'}
            </div>
          )}
        </div>
      </div>
